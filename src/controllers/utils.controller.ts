...
export async function uploadPublicWebsiteImageHandler(req: Request, res: Response) {
  const functionName = uploadPublicWebsiteImageHandler.name;
  const { file } = req;
  if (!file) {
    return SendErrorResponse.badRequest({
      res,
  ...
  const fileUploadRes = await uploadFileR2WithAutoKey(filepath, "public-images", false);
